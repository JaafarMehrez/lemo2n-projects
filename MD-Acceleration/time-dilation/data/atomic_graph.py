...
1. Handle partial charges (node attribute) - Added by : Jaafar Mehrez
...
from atomic_computes import find_edges_for_periodic_system
...
class AtomicGraph(Data):
    ...
    def from_atoms_dict(
        ...
        atomic_num = atoms_dict.get(ATOMIC_NUMBERS_KEY)
        ...
        # --- NEW: handle partial charges (node attribute) ---
        if PARTIAL_CHARGES_KEY in atoms_dict:
            pc = atoms_dict[PARTIAL_CHARGES_KEY]
            # convert to torch tensor if necessary and ensure shape (N,1)
            if not isinstance(pc, torch.Tensor):
                import numpy
                pc_arr = numpy.asarray(pc)
                # flatten to (N,) then view as (N,1)
                pc_t = torch.tensor(pc_arr.reshape(-1), dtype=torch.get_default_dtype())
            else:
                pc_t = pc.clone().detach().to(dtype=torch.get_default_dtype())
            kwargs[PARTIAL_CHARGES_KEY] = pc_t.view(-1, 1)
        # if not present: we simply don't add that field (AtomicGraph instances without it are allowed)
